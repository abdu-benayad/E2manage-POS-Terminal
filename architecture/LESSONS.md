...
| [A green suite over the wrong picture](./lessons/verification-and-false-greens.md#a-green-suite-over-the-wrong-picture--2026-08-25) | Eight both-directions tests, all green and all correct, over a screen rendering **light widgets on a black panel** with the heading invisible on two phases. A panel fill reaches **no accessibility node**, so no query could have failed — the boundary of the instrument, not a gap in the tests. Cause: `abdu-egui-ui` quarantines egui's `Visuals` as the host's to own, and egui's default preference **falls back to dark with no OS signal**, which is what a till and a headless harness both are. Two further findings from the same glance: a snapshot that builds its own `Environment` pins a configuration **the binary does not use** (hence `screen::chrome()`, called by both); and `UPDATE_SNAPSHOTS=1` prints `ok` over anything, which is how a wire token — `as_wire_str`, `SUPERVISOR` under an Arabic name — was caught that layer 1 **agreed with**, because the tree carries the string actually drawn. Plus: a copied `kittest.toml` threshold is a property of someone else's pictures (measured here: floor 0, break 244, set 20). |
