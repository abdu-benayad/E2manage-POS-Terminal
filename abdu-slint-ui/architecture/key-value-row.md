...
| `wrap`          | `bool`         | `false`    | Applies to `label` and `description` only. When `false` they stay single-line; when `true` they wrap and the row grows vertically (height lock relaxes). `value`, `value-unit`, and `value-icon` always stay single-line — Slint's word-wrap treats `.` in numeric values like `42.00` as a word boundary, producing character-level breaks that look like a vertical column of digits. Long values overflow horizontally instead. |
